...
import { BMapComponent } from '../common';
...
class Map extends BMapComponent {
  /** div的element实例 */
  @Ref('div-bmap') readonly el!: HTMLDivElement;
  ...
  updated(prevProps: MapProps) {
    let { center: preCenter, zoom: preZoom } = prevProps;
    let { center, zoom } = this.getOptions();
  ...
  initialize(): void {
    ...
    // todo 地图的默认props呢？
    let props = this.$props as MapProps;

    // 创建Map实例，defaultOptions优先级低于指定配置
    // todo  this.defaultOptions? 默认的配置怎么办
    // let options = Object.assign({}, this.defaultOptions, this.getOptions());
    let options = Object.assign({}, this.getOptions());
    ...
    if (props.mapStyleV2) {
      options.style = props.mapStyleV2;
    }
    let map = new BMapGL.Map(this.el, options as BMapGL.MapOptions);
    ...
    if (isString(props.center)) {
      map.centerAndZoom(props.center, props.zoom);  // 初始化地图,设置中心点坐标和地图级别
    } else {
      let { lng, lat } = props.center as BMapGL.Point;
      let center = new BMapGL.Point(lng, lat);
      map.centerAndZoom(center, props.zoom);  // 初始化地图,设置中心点坐标和地图级别
    }

    if (props.heading) {
      map.setHeading(props.heading);
    }
    if (props.tilt) {
  ...
  render() {
    return (
      <div>
        <div ref="div-bmap" style={{ height: '100%', width: '100%' }}>
          加载地图中...
        </div>
      </div>
    );
